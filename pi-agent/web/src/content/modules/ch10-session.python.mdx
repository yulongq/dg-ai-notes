...
最直觉的答案：**线性数组**。`messages = [msg1, msg2, msg3, ...]`，一条接一条。这种结构最简单，也接近常见 messages 表按时间排序后的形态。
...
### 一个简化的 MessageEntry 长这样

第 3 步产生的 AssistantMessage，在 `.jsonl` 文件中的结构大致如下。为了突出树字段，示例使用便于阅读的短 id，并省略了 `api`、`provider` 和完整 usage 明细；真实 v0.80.2 AssistantMessage 会携带这些字段，entry id 通常是 8 位十六进制字符串：
...
第 2 节的例子出现了一个 `model_change` entry，以及 `MessageEntry` 中的 user、assistant、toolResult 三种标准消息角色。coding-agent 一共定义了 9 种 Entry 类型；按“对重建结果的影响”分三组更容易理解：
...
| 类型 | 产生什么消息 | 例子 |
|------|------------|------|
| `MessageEntry` | 标准消息，以及允许直接持久化的 coding-agent 消息 | User / Assistant / ToolResult；直接执行的 BashExecution |
...
**配图说明**：所有 Entry 共享基础字段（type / id / parentId / timestamp），按“对重建结果的影响”分三组——① 贡献上下文内容（4 种，红色）；② 影响状态（2 种，黑色，更新 model / thinkingLevel）；③ 纯元数据（3 种，虚线灰色，`buildSessionContext()` 跳过）。这个分类对应下一节的派发逻辑。
...
**没有修改 e2**——只是创建 e3 时让它的 parentId 指向 e2。`byId` 用于按 id 找节点；需要完整树时，`getTree()` 扫描 entry，并借助节点映射把每个 child 挂到其 parent 下。
...
有时候你希望当前路径的 Agent **大致知道**之前试过什么——不用看完整对话，知道个大概就行。比如“之前的尝试发现 salt 编码有问题，但没解决根因”。高层入口是 `AgentSession.navigateTree(targetId, { summarize: true })`：它收集旧路径、生成摘要，再把已经生成好的摘要交给 `SessionManager.branchWithSummary()`。
...
**这是可选的**——不需要摘要时，树导航最终只调用 `branch()`（或回到根时调用 `resetLeaf()`）。需要摘要时，分支专用 prompt 负责生成内容；`branchWithSummary()` 自身只校验挂载点并追加 `BranchSummaryEntry`，不会调用模型。
...
树长好了，但 **LLM 不认识树**。LLM 的 API 接受一个**线性 messages 数组**（第 6 章讲过：user / assistant / toolResult 三种）。coding-agent 在恢复会话，或在树导航、压缩等操作后重建 Agent 状态时，用 `buildSessionContext()` 把当前路径“压扁”；普通的后续模型请求直接使用已经维护在 `agent.state.messages` 中的线性状态，并不会每次都重扫整棵树。
...
第 2 节那个会话，在磁盘上的结构可以简写成下面这样。为方便对应示意树，这里继续使用 e1、e2 这样的教学 id，并省略 AssistantMessage 的部分必需字段；真实 entry 通常使用 8 位十六进制 id，且每一行都是完整 JSON：
