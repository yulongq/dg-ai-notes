...
```python
# ============================================================
# 【Python 改写】packages/ai/src/types.ts:427-431
# 原文 TS:
...
# 概念对照：内层 except 只处理 ops.exec 抛出的中止/超时；
# 非零退出码在正常返回后单独检查，外层 finally 始终清理 timer。
try:
    try:
...
```python
# 原文 TS: catch 后返回 createErrorToolResult(...)，并设置 isError: true
def encode_tool_error(error: Exception) -> dict:
    return {
        "result": create_error_tool_result(str(error)),
        #                            ^^^^^^^^^^
...
```python
# 原文 TS: execute 内识别已知异常并重写消息，未知异常原样抛出
...
class ReadOperations(Protocol):
    ...
    # TS 的 null / undefined 在这段概念映射里都落到 Python 的 None；
...
```python
# ============================================================
# 【Python 改写】创建 Read 工具时捕获 ops，execute 通过它间接调用
# 原文 TS:
...
def create_read_tool_definition(cwd, options=None):
...
# 原文 TS:
#   // 本地执行（默认）
#   const localTool = createReadToolDefinition(cwd);
#   // 单元测试（Mock）
...
#       }
#   });
#   // 远程执行（ssh 提供匹配 ReadOperations 的 Promise 接口）
...
class SSHReadOperations:
    async def read_file(self, absolute_path): return await ssh.read_file(absolute_path)
    async def access(self, absolute_path):         await ssh.access(absolute_path)
