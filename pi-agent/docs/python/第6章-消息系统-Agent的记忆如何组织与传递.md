...
这一章就来回答这些问题。你会看到 Pi 消息系统最核心的设计——**两层消息**：Agent 内部用丰富的格式自由表达，到了 pi-ai 适配边界投影为统一 `Message`，再由 Provider 适配器翻译成各家私有协议。
...
但问题来了：当 Agent Loop 准备调用模型时，**pi-ai 的统一适配边界**根本不接受 `BashExecutionMessage`。这个边界只接受三种 `Message`：`user`（用户说的）、`assistant`（AI 回的）、`toolResult`（工具返回的）。BashExecutionMessage 不属于这三种中的任何一种；再往下，各 Provider 适配器还会把这三种内部消息翻译成自己的 API 协议。
...
> **进阶细节**：content 块里还有一些 `*Signature` 字段（`textSignature`、`thinkingSignature`、`thoughtSignature` 等），是某些 Provider 要求的不透明 ID 或密文，下一轮请求可能需要原样回传。日常理解不需要深究，知道它们服务于**同一 Provider 协议内**的上下文连续性即可；它们不是跨 Provider 通用的签名格式。
...

{
    ...
    "is_error":    bool,                                       # 工具边界是否把本次调用标记为失败
    "timestamp":   int,
}
```
...
│       content: [
│           { type: "text", text: "import { auth } from '...' ..." }
│       ]
...
└── [3] AssistantMessage
        role: "assistant"
        content: [
            { type: "text", text: "auth.ts 是一个认证模块..." }
        ]
        stopReason: "stop"        ← 没调工具，循环结束
```

这就是 pi-ai 统一消息协议表达的世界——用户说了什么、AI 回了什么、工具返回了什么，就这么三种。真实 Provider API 看到的角色和内容块，还要经过下一层适配。
...
这里其实藏着一个更普遍的问题：**Agent 内部除了"LLM 对话"这件事，还有大量功能性数据要管理**——Bash 命令的执行记录、上下文被压缩后的摘要、同一 Session Tree 上被放弃路径的摘要、用户上传的附件元信息……
...
Pi 的设计是**两边都不妥协**：**以结构化形式存进 Agent 状态和 Session**（满足 UI/持久化），每次 run 再复制成 `AgentContext` 快照；**在交给 pi-ai 适配器前做一次投影**（满足模型调用）。这样 UI 永远有完整的结构化数据可用，LLM 也能看到它需要的扁平版本。投影是在请求边界发生的、有损的、单向的——损失掉的结构化字段仍保留在状态和 Session 中。
...
├── BranchSummaryMessage       ← 同一 Session Tree 中被放弃分支的摘要
...
**3. 精细化可见性控制**。因为自定义消息有自己的 `role`，可以在 `convertToLlm` 翻译时做特殊处理。coding-agent 的 `BashExecutionMessage` 就带有 `excludeFromContext` 字段：为 `true` 时，内置转换器不把它发给 LLM，但 UI 仍可渲染。pi-ai `Message` 没有这个内置字段；宿主当然仍可在 `transformContext` 中过滤任意消息，只是要自行定义规则。
...
![两层消息——内部结构与 Provider 适配边界](assets/260702-ch06-two-layer-messages.svg)

**配图说明**：顶部 AgentMessage 联合类型一分为二——左支 Message（3 种统一消息）、右支 CustomAgentMessages（4 种扩展）。中间红框是 `convertToLlm()` 投影门。底部 pi-ai 适配器只接收 3 种统一消息；自定义消息要么被过滤、要么被投影成 UserMessage，之后才由 Provider 适配器继续翻译。
...
翻译成人话：**AgentMessage = pi-ai 统一消息 + 自定义消息。** 它是 Message（三种统一格式）和 CustomAgentMessages（自定义扩展）的联合类型。
...
│
├── Message（可交给 pi-ai Provider 适配器的统一消息）
│   │
...
Agent Loop 每次 run 使用的 `context.messages` 快照是 `AgentMessage[]`——它可以混合存放统一消息和自定义消息。一条消息到底是哪种成员，看 `role` 字段就能判断。
...
这段代码的效果是：**编译器自动把这 4 种类型加入 AgentMessage 联合类型。** 以后在 coding-agent 项目里，`AgentMessage` 就变成了 7 种消息的联合（3 种 pi-ai `Message` + 4 种自定义），TypeScript 会帮你做完整的类型检查。
...
## 五、转换边界：convertToLlm —— 自定义消息被投影或过滤

现在我们知道 Agent 内部用 7 种消息类型自由表达。但每次调用模型时，pi-ai 的 Provider 适配器入口只接受 3 种 `Message`。怎么办？
...
[2] convertToLlm (必须)                 ← AgentMessage[] → Message[]
        │                                  自定义消息投影成统一格式
        ▼
llmContext.messages: Message[]          ← pi-ai 适配器接收的统一消息
        │
        ▼
streamFunction(model, llmContext, ...)  ← 适配成 Provider 私有协议并调用模型
```
...
为什么可见的自定义消息都变成 `user`？这些内容代表宿主或用户一侧注入给模型的信息，而不是一次模型输出。转换后可能出现连续的 user 消息；`convertToLlm` 在 v0.80.2 中不会合并它们，是否需要规范化由后续 Provider 适配器处理。
...
**After —— UserMessage（pi-ai 适配器收到的统一格式）：**
...
- `command` 和 `output` 总会被格式化进文本；无输出时会写成 `(no output)`
- 其余状态是**条件性序列化**：`cancelled=true` 才追加取消说明；否则仅非零 `exitCode` 会追加退出码；`truncated=true` 且存在 `fullOutputPath` 时才追加完整输出路径
- 这些字段即使出现在文本里，也不再保留独立的结构化形态；示例中的 `exitCode=0`、`cancelled=false`、`truncated=false` 因此不会产生额外文字
...
**配图说明**：横向数据流——AgentMessage[7] → transformContext（同层变换，类型不变）→ convertToLlm（跨层翻译）→ Message[3] 交给 pi-ai Provider 适配器。下方标注 excludeFromContext 过滤分支；Provider 私有协议翻译发生在图的下一层。

回到管道图，有一个设计细节值得追问：**为什么分两步，而不是一步到位？**

答案是**职责分离**：

- **transformContext** 处理的是 **AgentMessage 级别的操作**：过滤、重排或注入请求级上下文。它处理前后都是 `AgentMessage[]`，类型不变。v0.80.2 的 coding-agent 在这里调用扩展系统的 context hook；内置 Compaction 的触发、摘要生成、Session Tree 写入和 `agent.state.messages` 替换都发生在 AgentSession 层，不是这个钩子完成的。
- **convertToLlm** 处理的是 **跨类型翻译**：把 AgentMessage 翻译成 Message。处理前是 `AgentMessage[]`，处理后是 `Message[]`，类型变了。

分开的好处是：许多变化可以只落在其中一个边界上。

- 换了**请求级上下文策略**（比如过滤某类临时消息、注入 Git 状态），可以只改 `transformContext`；`convertToLlm` 不需要动。若策略还要持久化摘要或重建 Session Tree，就不能只靠这个请求级钩子，coding-agent 的内置 Compaction 正是由 AgentSession 另行编排。
...
这里有一个容易混淆的点要强调：**换了 LLM 提供商（比如从 Claude 换成 GPT），convertToLlm 不需要动**。为什么？因为 `convertToLlm` 的输出是统一的 `Message[]`（3 种统一消息），它已经把“自定义消息 → pi-ai 消息”这件事做完了。再往下，**把 Message 翻译成各家 Provider 的私有格式**是 pi-ai 层（第 4 章讲过）的工作——那一层有自己的翻译器（anthropic-messages、openai-completions 等），跟 convertToLlm 是完全独立的两层。换句话说：**Pi 把“宿主消息投影”和“Provider 协议翻译”放在了两层不同的抽象里，互不干扰**。
...
注意：`excludeFromContext = true` 的消息**仍然存在于 Agent 状态和 Session 中**；run 内生成的 `AgentContext` 快照也不会被这个转换就地改写。UI 仍然可以看到它、渲染它。只是在构造本次 pi-ai `Message[]` 时，这条消息被“隐身”了。

这就是"UI 能看、LLM 不能看"的机制——一个布尔字段，在翻译边界做过滤，数据本身不需要删除。

### v0.80.2 内置的三种可见性

就 coding-agent 已声明的 7 种消息而言，这里可以观察到三种内置行为。除了 `bashExecution.excludeFromContext` 控制“只给 UI 看”，`CustomMessage.display` 还控制 TUI 与 HTML 导出是否渲染；它不影响 `convertToLlm`：
...
| UI 不展示 | 是 | 否 | `CustomMessage.display = false`；仍转换成 UserMessage | 扩展注入的隐藏上下文 |

扩展可以再定义自己的消息与过滤规则，但“仅持久化、两边都不可见”不是本章这份 v0.80.2 内置类型表中的现成类别。这里的“UI 是否看到”特指 coding-agent 的交互 TUI / HTML 导出；RPC 的原始事件消费者仍可自行观察消息。
...
[1] 创建消息
    BashExecutionMessage {
        ...
        │
        ▼
[2] 存入 agent.state.messages，并写入 session
    [...原有消息, 新的 BashExecutionMessage]
        │
        ▼
[3] Bash 命令处理结束；这里不会自动调用 LLM
        │
        ▼
[4] 用户稍后提交普通 prompt，Agent 创建 AgentContext 快照并启动 Loop
    快照中包含此前的 BashExecutionMessage
        │
        ▼
[5] transformContext（可选）
...
[6] convertToLlm（必须）
    ...
    excludeFromContext → 过滤掉
        │
        ▼
[7] pi-ai Provider 适配器收到统一 Message[]
    llmContext.messages = [
        ...之前的消息,
        {
            role: "user",
        ...
        │
        ▼
[8] Provider 适配器翻译成私有协议并调用模型
        │
        ▼
[9] LLM 回复
    → 产生新的 AssistantMessage
    → 可能触发工具调用
    → ToolResultMessage（第 5 章的五步管道）
    → 写回 Agent 状态，继续循环
```

**核心规律**：coding-agent 在 v0.80.2 中使用 7 种消息类型（3 种 pi-ai `Message` + 4 种自定义），到 pi-ai 适配边界时，自定义消息被过滤或投影为 3 种 `Message`。这一步不会直接产生某家 API 的请求体；Provider 兼容性由下一层适配器完成。
...
- **AI 抽象层的 Provider 适配入口**只接收 Pi 归一化后的三种消息（User/Assistant/ToolResult）；不同 Provider 的真实协议还会在下一层继续翻译
...
| 层 | 关心谁 | 数据形式 | 怎么实现 |
|----|--------|---------|---------|
| **AgentMessage（内层）** | 功能层 | coding-agent 声明 7 种消息（3 种 pi-ai `Message` + 4 种自定义），字段丰富 | 用联合类型 + 声明合并，避免 agent-core 依赖具体宿主，同时保留类型检查 |
| **Message（外层）** | pi-ai Provider 适配器 | 3 种统一消息，字段精简 | 在 Provider 适配前做一次 `convertToLlm` 翻译，**有损、单向、最后一刻发生** |
...
从第 7 章开始进入进阶章节。回看前五章，有一个东西反复出现但我们始终没深入：**事件**。第 3 章说“Agent Loop 每做一步都发事件让 UI 实时更新”，第 5 章说“工具执行时发出 `tool_execution_start`、`tool_execution_update`、`tool_execution_end` 事件”。这些事件怎么从 Agent 内部传到外部？agent-core 为什么等待自己的监听器？这个等待跨过 AgentSession 后，是否还会覆盖 UI 的异步回调？
