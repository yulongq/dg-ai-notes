...
# 只有整批工具都 terminate，has_more_tool_calls 才为 False

tool_calls = [c for c in message.content if c.type == "toolCall"]
has_more_tool_calls = False
if len(tool_calls) > 0:
    executed_tool_batch = await execute_tool_calls(...)
    has_more_tool_calls = not executed_tool_batch.terminate  # 整批工具都 terminate 才停止
```
...
async def consume_response(response, context, emit):
    async for event in response:
        if event.type == "start":
            ...
            await emit({"type": "message_update", "message": partial_message})
