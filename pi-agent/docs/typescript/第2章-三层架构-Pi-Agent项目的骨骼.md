...
```typescript
// packages/agent/src/index.ts（结构摘要；省略了其他逐项导出）
...
```typescript
// packages/coding-agent/src/cli.ts（核心调用节选）
#!/usr/bin/env node
...
// 未经 wrapper 不能直接作为 AgentTool（详见 types.ts:435）
...
```typescript
// 只用底层；从 Provider / Models 入口调用，不依赖 Agent
